@extends('layouts.app', ['pageTitle' => 'Register', 'contentWidth' => 'max-w-3xl'])
...
@section('content')
    ...
    <section class="py-16 sm:py-24">
        <div class="{{ $cardClass }}">
            <p class="text-sm font-semibold uppercase tracking-[0.28em] text-[var(--text-muted)]">Create your workspace</p>
            <h1 class="mt-4 text-4xl font-semibold tracking-[-0.04em] text-[var(--text-primary)]">Launch your own private assistant</h1>
            <p class="mt-4 text-base leading-7 text-[var(--text-secondary)]">
                Create an account, upload your own documents and URLs, and get AI answers grounded only in your own knowledge.
            </p>
            ...
            <form method="POST" action="{{ route('register.store') }}" class="mt-10 space-y-6">
                ...
                <div>
                    <label for="name" class="mb-2 block text-sm font-medium text-[var(--text-primary)]">Name</label>
                    <input id="name" name="name" type="text" value="{{ old('name') }}" required autofocus class="{{ $inputClass }}" />
                    @error('name')
                ...
                <div>
                    ...
                    <input id="email" name="email" type="email" value="{{ old('email') }}" required class="{{ $inputClass }}" />
                    @error('email')
                        <p class="mt-2 text-sm text-rose-400">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div>
                    <label for="password_confirmation" class="mb-2 block text-sm font-medium text-[var(--text-primary)]">Confirm password</label>
            ...
            <p class="mt-8 text-sm text-[var(--text-secondary)]">
                ...
                <a href="{{ route('login') }}" class="font-medium text-[var(--brand-primary)]">Log in</a>
            </p>
        </div>
    </section>
@endsection
